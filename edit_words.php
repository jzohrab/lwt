...
case 'term,rom,transl':
    $wh_query =' and (WoText ' . $wh_query . ' or IFNULL(WoRomanization,\'*\') ' . 
    $wh_query . ' or WoTranslation ' . $wh_query . ')';
    break;
case 'term,rom':
    $wh_query = ' and (WoText ' . $wh_query . ' or IFNULL(WoRomanization,\'*\') ' . 
    $wh_query . ')';
...
if (!$no_pagestart) {
    pagestart(
        'My ' . getLanguage($currentlang) . ' Terms (Words and Expressions)', true
    );
}
    ...
    if (isset($_REQUEST['marked'])) {
        if (is_array($_REQUEST['marked'])) {
            ...
            if ($l > 0 ) {
                ...
                if ($markaction == 'del') {
                    $message = runsql(
                        'delete from words where WoID in ' . $list, 
                        "Deleted"
                    );
                    do_mysqli_query(
                        'update textitems2 
                    ...
                    flexible_export(
                        'select distinct WoID, LgName, LgExportTemplate, 
...
if (isset($_REQUEST['allaction'])) {
    ...
    if (
        ...
        $allaction == 'smi1all' || $allaction == 's5all' || $allaction == 's1all' || 
        $allaction == 's99all' || $allaction == 's98all' || 
        $allaction == 'todayall' || $allaction == 'addtagall' || 
        $allaction == 'deltagall' || $allaction == 'delsentall' || 
        $allaction == 'lowerall' || $allaction == 'capall'
    ) {
        if ($currenttext == '') {
            $sql = 'select distinct WoID from (
                ...
                left JOIN wordtags 
                ON WoID = WtWoID
            ) where (1=1) ' . $wh_lang . $wh_stat .  $wh_query . ' 
            ...
            Ti2TxID in (' . $currenttext. ')' . $wh_lang . $wh_stat . $wh_query . 
            ' group by WoID ' . $wh_tag;
        }
        $cnt=0;
        ...
        if ($currenttext == '') {
            ...
            anki_export(
                ...
                from ((words left JOIN wordtags ON WoID = WtWoID) left join tags on TgID = WtTgID), languages, textitems2 where Ti2LgID = WoLgID and Ti2WoID = WoID and Ti2TxID in (' . $currenttext . ')' . ' and WoLgID = LgID AND WoTranslation != \'*\' and WoSentence like concat(\'%{\',WoText,\'}%\') ' . $wh_lang . $wh_stat . $wh_query . ' group by WoID ' . $wh_tag);
        }
        ...
        if ($currenttext == '') {
            tsv_export(
                'select distinct WoID, LgName, WoText, WoTranslation, 
                WoRomanization, WoSentence, WoStatus, 
                ifnull(group_concat(distinct TgText order by TgText separator \' \'),\'\') as taglist from ((words left JOIN wordtags ON WoID = WtWoID) left join tags on TgID = WtTgID), languages where WoLgID = LgID ' . $wh_lang . $wh_stat .  $wh_query . ' group by WoID ' . $wh_tag);
        } else {
            tsv_export(
                'select distinct WoID, LgName, WoText, WoTranslation, 
                WoRomanization, WoSentence, WoStatus, 
                ifnull(group_concat(distinct TgText order by TgText separator \' \'),\'\') as taglist from ((words left JOIN wordtags ON WoID = WtWoID) left join tags on TgID = WtTgID), languages, textitems2 where Ti2LgID = WoLgID and Ti2WoID = WoID and Ti2TxID in (' . $currenttext . ')' . ' and WoLgID = LgID ' . $wh_lang . $wh_stat . $wh_query . ' group by WoID ' . $wh_tag);
        }
        ...
        if ($currenttext == '') {
            $sql = 'select distinct WoID 
            from (words left JOIN wordtags ON WoID = WtWoID) 
            where (1=1) ' . $wh_lang . $wh_stat .  $wh_query . ' group by WoID ' . $wh_tag;
        } else {
            $sql = 'select distinct WoID 
            from (words left JOIN wordtags ON WoID = WtWoID), textitems2 
...
if (isset($_REQUEST['new']) && isset($_REQUEST['lang'])) {
    ...
    // CHG
    $sql = 'select * from words, languages 
...
<option value="term"<?php if($currentquerymode=="term") { echo ' selected="selected"'; 
} ?>>Term</option>
<option value="rom"<?php if($currentquerymode=="rom") { echo ' selected="selected"'; 
} ?>>Romanization</option>
<option value="transl"<?php if($currentquerymode=="transl") { echo ' selected="selected"'; 
} ?>>Translation</option>
<option disabled="disabled">------------</option>
<option value="term,rom"<?php if($currentquerymode=="term,rom") { echo ' selected="selected"'; 
} ?>>Term, Rom.</option>
<option value="term,transl"<?php if($currentquerymode=="term,transl") { echo ' selected="selected"'; 
} ?>>Term, Transl.</option>
<option value="rom,transl"<?php if($currentquerymode=="rom,transl") { echo ' selected="selected"'; 
} ?>>Rom., Transl.</option>
</select><?php
